<x-layout :title="$project->title . ' | Surya Andika'" :show-back="true">
    ...
    <div class="pt-8"> {{-- offset for the fixed nav from x-nav --}}
        ...
        <main class="max-w-[1600px] mx-auto px-8 lg:px-20">
            <div class="reveal py-16 lg:py-20 border-b border-slate-100">
                <div class="max-w-3xl">
            ...
            <div class="reveal py-16 lg:py-20 space-y-12">
                <h3 class="text-2xl font-bold text-slate-900">Visual Showcase</h3>

                <button type="button"
                            class="group relative rounded-[2rem] sm:rounded-[3rem] overflow-hidden shadow-2xl border border-slate-200 bg-white cursor-zoom-in w-full text-left"
                            data-modal-trigger
                    ...
                    @if (! empty($project->galleryImages()))
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 sm:gap-8">
                            @foreach ($project->galleryImages() as $image)
                                <button type="button"
                                        style="--reveal-delay: {{ $loop->index * 70 }}ms"
                                        class="reveal bg-white p-4 rounded-[2rem] sm:rounded-[2.5rem] shadow-xl border border-slate-100/50 group cursor-zoom-in text-left w-full"
                                        data-modal-trigger
                                        data-image-src="{{ asset($image) }}"
                                        aria-label="View detail image {{ $loop->iteration }} of {{ $project->title }} fullscreen">
                                    <div class="aspect-[4/5] rounded-[1.5rem] sm:rounded-[1.8rem] overflow-hidden border border-slate-50 bg-slate-100">
                                        <img src="{{ asset($image) }}"
                                             loading="lazy"
                                             decoding="async"
    ...
    <div id="imageModal" class="hidden fixed inset-0 z-[100] bg-slate-950/95 backdrop-blur-sm items-center justify-center p-4 md:p-10" role="dialog" aria-modal="true" aria-label="Image preview">
        <button type="button" id="modalClose" class="absolute top-6 right-6 md:top-8 md:right-8 text-white hover:text-blue-400 transition transform hover:rotate-90 duration-300" aria-label="Close image preview">
        ...
        <img id="modalImg" src="" class="max-w-full max-h-full w-auto h-auto object-contain rounded-lg shadow-2xl" alt="">
    </div>
